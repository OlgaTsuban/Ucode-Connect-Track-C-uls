...
t_lout wc_getSizesForL(t_obj **fp, int fp_amt, bool *fl) {
    ...
    for (int i = 0; i < fp_amt; i++) {
        ...
        else if (!tmp && r.b_owner < getBitDepth(fp[i]->st.st_uid))
            r.b_owner = getBitDepth(fp[i]->st.st_uid);
        //Column 4 (group)
        tmp_g = getgrgid(fp[i]->st.st_gid);
        if (tmp_g && !fl[n] && r.c_group < mx_strlen_safe(tmp_g->gr_name))
            r.c_group = mx_strlen_safe(tmp_g->gr_name);
        else if (!tmp_g && r.c_group < getBitDepth(fp[i]->st.st_gid))
            r.c_group = getBitDepth(fp[i]->st.st_gid);
        // Column 5 (size)
        if (fp[i]->type == chr || fp[i]->type == blk || fp[i]->type == sock) {
            r.d_size = r.d_size < 9 ? 9 : r.d_size;
            r.d_minor = true;
        }
        else if (r.d_size < getBitDepth(fp[i]->st.st_size))
            r.d_size = getBitDepth(fp[i]->st.st_size);
        r.total += fp[i]->st.st_blocks;
    }
    r.a_link = getBitDepth(link);
    return r;
}
