...
static inline void printH(struct stat st) {
    mx_printstr("   ");
    if (st.st_size < 1000) {
        for (int i = 3 - getBitDepth(st.st_size); i > 0; i--)
            mx_printchar(' ');
        mx_printint(st.st_size);
        mx_printchar('B');
    }
    else if (st.st_size >= 1000 && st.st_size < 1000000) {
        printDouble((double)st.st_size / BIBYTE);
        mx_printchar('K');
    }
    else if (st.st_size >= 1000000 && st.st_size < 1000000000) {
        printDouble((double)st.st_size / BIBYTE / BIBYTE);
        mx_printchar('M');
    }
    else if (st.st_size >= 1000000000 && st.st_size < 1000000000000) {
        printDouble((double)st.st_size / BIBYTE / BIBYTE / BIBYTE);
        mx_printchar('G');
    }
}
static inline void printSize(struct stat st, t_lout l, bool *fl) {
    ...
    if (S_ISCHR(st.st_mode) || S_ISBLK(st.st_mode) || S_ISSOCK(st.st_mode)) {
        ...
        //major
        for (int i = l.d_size - getBitDepth(mjr) - 4; i > 0; i--)
            mx_printchar(' ');
        mx_printint(mjr);
        mx_printstr(", ");
        //minor
        for (int i = l.d_size - getBitDepth(mnr) - 6; i > 0; i--)
            mx_printchar(' ');
        if (mnr > 255) {
        ...
        if (fl[h])
            ...
            for (int i = l.d_size - getBitDepth(st.st_size) + !l.d_minor; i > -1; i--)
                mx_printchar(' ');
            mx_printint(st.st_size);
        }
    }
...
void printStats(struct stat st, t_lout l, bool *fl) {
    ...
    //Column 2 (links)
    for (int i = l.a_link - getBitDepth(st.st_nlink); i >= 0; i--)
        mx_printchar(' ');
    ...
    if (!fl[g]) {
        ...
        if (tmp && !fl[n]) {
            ...
            for (int i = l.b_owner - getBitDepth(st.st_uid); i > 0; i--)
                mx_printchar(' ');
        }
    }
    // Column 4 (group)
    if (!fl[o]) {
        ...
        if (tmp_g && !fl[n]) {
            ...
            for (int i = l.c_group - getBitDepth(st.st_gid); i > 0; i--)
                mx_printchar(' ');
        }
    }
