...
static inline void printObjArr(t_obj **fp, int fp_amt, bool *fl, bool use_t) {
    if (!fp_amt)
        return;
    if (fl[one])
        printWithOne(fp, fp_amt, fl);
    else if (fl[m])
        printWithM(fp, fp_amt, fl);
    else if (fl[l]) {
        printWithL(fp, fp_amt, fl, use_t);
    } else {
        printWithC(fp, fp_amt, fl);
    }
}
        ...
        for (int i = 0; i < obj->kids_amt; i++) {
            ...
            if (obj->kids[i]->type == dir) {
                ...
                fl[F] || fl[p] ? write(1, obj->kids[i]->path_name, len - 1) :
                                 write(1, obj->kids[i]->path_name, len);
                mx_printstr(":\n");
                errorPermDenied(obj->kids[i]->path_name);
            }
        }
}

void printResult(t_data *d, bool *fl) {
    int len;
    if (d == NULL)
        return;
    ...
    if (d->dirs_amt > 1 || d->files_amt) {
        ...
        for (int i = 0; i < d->dirs_amt; i++) {
            ...
            if (d->dirs_path[i]->type != perm_denied)
                ...
                errorPermDenied(d->dirs_path[i]->path_name);
        ...
        if (d->dirs_path[0]->type == perm_denied)
            errorPermDenied(d->dirs_path[0]->path_name);
        else
